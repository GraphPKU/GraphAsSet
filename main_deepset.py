...
import numpy as np
from ONDeepSet import ONDeepSet
import os
...
def parserarg():
    ...
    parser.add_argument("--nogaggr_res", dest="gaggr_res", action="store_false")
    parser.add_argument("--nogaggr_mlptailact1", dest="gaggr_mlptailact1", action="store_false")
    ...
    parser.add_argument("--notprod_mlptailact", dest="tprod_mlptailact", action="store_false")
    parser.add_argument("--tprod_mlplayers", type=int, default=1)
    parser.add_argument("--tprod_mlpnorm", type=str, default="none")
...
def buildModel(args, num_tasks, device, dataset, needcompile: bool=True):
    ...
    kwargs = {
        ...
        "gaggr": {
            "permlayer":{
                ...
                "pool": args.gaggr_aggr
            }
        },
        "tprod": {
            "mlp":{
                "numlayer": args.tprod_mlplayers,
                "norm": args.tprod_mlpnorm,
                "tailact": args.tprod_mlptailact,
            }
        },
        "predlin": {
    ...
    if args.nodetask:
        args.pool = "none"
    model = ONDeepSet(args.featdim, args.caldim, args.hiddim, num_tasks, args.num_layers, args.pool, **kwargs)
